...
func (r *GslbReconciler) createGSLBFromIngress(c client.Client, a client.Object, strategy string) {
	...
	if errors.IsNotFound(gslbExistErr) {
		...
		gslbEmpty.Spec.Strategy, err = parseStrategy(a, strategy)
		if err != nil {
			log.Err(err).Msg("Ingress handler: parsing strategy error")
		}
	...
	log.Info().Msgf("XXXXX gslb.Spec.Strategy %v", gslb.Spec.Strategy)
	gslb.Spec.Ingress = k8gbv1beta1.FromV1IngressSpec(ingressToReuse.Spec)
	gslb.Annotations = a.GetAnnotations()
	delete(gslb.Annotations, "kubectl.kubernetes.io/last-applied-configuration")
...
func parseStrategy(a client.Object, strategy string) (result k8gbv1beta1.Strategy, err error) {
