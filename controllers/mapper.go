...
func (m *mapper) updateGslbStatus(gslb *k8gbv1beta1.Gslb) error {
	upstreamGslb, r, err := m.getGslb(gslb.Namespace, gslb.Name)
	if r != mapperResultUpdate {
		return fmt.Errorf("updating Gslb status failed (%v)", err)
	}
	upstreamGslb.Status = *gslb.Status.DeepCopy()
	return m.Client.Status().Update(context.TODO(), upstreamGslb)
}
